...
import { LineObject, Header, Configuration, Global }    from './types.ts';
...
function handle_scribejs(line_object: LineObject, config: Global): boolean {
    if (line_object.content_lower &&
       (line_object.content_lower.startsWith('scribejs, ') || line_object.content_lower.startsWith('sjs, '))) {
        ...
        try {
            ...
            case 'set': {
                ...
                if (name_comps.length !== 0) {
                    // Per the program logic this should never happen, but deno nags about it...
                    if (!config.nicks) config.nicks = [];
 ...
 */
export function get_label(line: string): {label: string|null, content: string} {
...
export function get_name_list(current_list: string[], line: LineObject, category :string, remove = true): undefined | string[] {
    ...
    const lower    = line.content_lower ? line.content_lower.trim() : "";
...
export function separate_header(lines: LineObject[], date: string): {headers: Header, lines: LineObject[]} {
    ...
    const single_item = (category: string, line: LineObject): boolean => {
         ...
         */
        const get_labelled_item = (label: string): string|null => {
