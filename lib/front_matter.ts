...
import { Global, Header }         from './types.ts';
import { Constants, Resolution }  from './types.ts';
...
function generate_metadata(header: Header, global: Global): string {
    ...
    if (global.action_list !== undefined && global.action_list.valid && global.action_list.actions.length > 0) {
        const repo_name = global.action_list.repo_name;
    ...
    if (global.resolution_list !== undefined && global.resolution_list.length > 0) {
