...
import { generate_front_matter }                from './front_matter.ts';
import { IssueReference, Global }               from './types.ts';
import { PersonWithNickname, Person }           from './types.ts';
import { LineObject, Header }                   from './types.ts';
...
export class Converter {
    ...
    private get_name(nick: string): Person {
        ...
        const nick_mapping = (the_nick: string) => {
            if (this.global.nicks === undefined) return null;
        ...
        if (this.global.nick_mappings !== undefined) {
            if (this.global.nick_mappings[clean_nick]) {
                return this.global.nick_mappings[clean_nick];
            } else {
                const person: PersonWithNickname | null = nick_mapping(clean_nick);
                if (person) {
...
`;
    ...
    private async generate_content(lines: LineObject[]): Promise<string> {
        ...
        */
        ...
        const add_to_resolutions = (content: string): string => {
            ...
            if (this.global.resolution_list !== undefined) {
        ...
        const add_to_actions = (content:string ): string => {
            ...
            if (words[1] === 'to') {
                ...
                if (this.global.action_list !== undefined && ghname !== undefined) {
    ...
    async convert_to_markdown(body: string|string[]): Promise<string> {
        ...
        if (this.global.action_list) this.global.action_list.set_date(headers.date);
